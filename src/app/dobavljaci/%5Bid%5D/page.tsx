...
export default function UrediDobavljacaPage() {
  ...
  useEffect(() => {
    async function loadSupplier() {
      ...
      if (supplier) {
        setFormData({
          nazivTvrtke: supplier.nazivTvrtke,
          oib: supplier.oib || "",
          kontaktOsoba: supplier.kontaktOsoba || "",
          email: supplier.email || "",
          telefon: supplier.telefon || "",
          adresa: supplier.adresa || "",
          napomena: supplier.napomena || ""
        });
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/dobavljaci");
       ...
       alert("Došlo je do greške prilikom lokalnog spremanja dobavljača.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-500 ease-out">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">Uredi Dobavljača</h2>
          <p className="text-slate-500 mt-1">Ažurirajte podatke o partnerskoj tvrtki.</p>
        </div>
      </div>
